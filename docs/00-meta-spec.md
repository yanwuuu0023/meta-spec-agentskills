...
1. Discuss tech stack + API. Skip design-system unless you ask. **Do not discuss app features or what to build — that belongs to `plan-m`.** If you mentioned an ambiguous concept (e.g., "swimming app" — tracking? lessons? social?), the agent clarifies the concept first; it never asks about features.
...
3. From that reading, discuss tech stack + API. Skip design-system unless you ask.
